...
class SumerianConnector {
    ...
    private static final String IMAGE_NAMES_TEXT_FILE = "image_names.txt";
    ...
    SumerianConnector(WebView webView, Session session, GLSurfaceView surfaceView) {
        ...
        imageNames = getImageNames(IMAGE_NAMES_TEXT_FILE);
    }


    ArrayList<String> getImageNames(String path){
        AssetManager am = this.mWebView.getContext().getAssets();
                ...
                try {
                  //  String imList[] = this.mSurfaceView.getContext().getAssets().list("awsimages");
                    for(int i = 0; i<imageNames.size();i++){
                        if (img.getName().equals(imageNames.get(i).split("\\.")[0])) {
                            imageAnchorCreated(img);
                            Log.d("AWS:", imageNames.get(i).split("\\.")[0]);
                            break;
                        }
                    }
                }
